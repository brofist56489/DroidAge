...
public class ConnectedTile extends Tile {
	...
	protected ConnectedTile(TileType tileType) {
		super(tileType);
	}

	public void render(int x, int y, World world) {
		ImageManager.renderFromTileMap("tileMap", x, y, data1, SIZE, 0);
	...
	private void renderInnerCorners(int x, int y, World world) {
		...
		if(ul) ImageManager.renderFromTileMap("tileMap", x, y, cornerId, SIZE, 0);
		if(ur) ImageManager.renderFromTileMap("tileMap", x, y, cornerId, SIZE, 1);
		if(dl) ImageManager.renderFromTileMap("tileMap", x, y, cornerId, SIZE, 2);
		if(dr) ImageManager.renderFromTileMap("tileMap", x, y, cornerId, SIZE, 3);
	}
